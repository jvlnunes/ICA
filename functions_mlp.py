...
def treinamento(X, Y, layers, epocas=10000, taxa_aprendizagem=0.1, precisao=1e-6):
    ...
    for epoch in range(epocas):
        ...
        for w, b in zip(weights, biases):
            z = np.dot(w, ativacoes[-1]) + b
            ativacoes.append(sigmoid(z))

        deltas = [ativacoes[-1] - Y.T]
        for i in range(len(layers) - 2, 0, -1):
            deltas.append(np.dot(weights[i].T, deltas[-1]) * derivada_sigmoid(ativacoes[i]))

        deltas.reverse()

        for i in range(len(weights)):
            weights[i] -= taxa_aprendizagem * np.dot(deltas[i], ativacoes[i].T)
